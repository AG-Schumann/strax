...
class Context:
    ...
    def search_field(self, pattern):
        ...
        for d in self._plugin_class_registry:
            if d not in cache:
                cache.update(self._get_plugins((d,), run_id='250'))
            p = cache[d]
    ...
    def data_info(self, data_name: str, run_id='0') -> pd.DataFrame:  # added run_id as kwarg since some runs don't have 'cathod_mean'
        """Return pandas DataFrame describing fields in data_name"""
        p = self._get_plugins((data_name,), run_id=run_id)[data_name]
